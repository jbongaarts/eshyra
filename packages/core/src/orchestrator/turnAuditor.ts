...
export function buildAuditSystemPrompt(): string {
  return [
    ...
    'authoritative) binds nobody, so treat that ambiguity as UNRESOLVED; an entry',
    'marked UNREPRESENTABLE ACTIVE CAMPAIGN RULE requires repair before it can be',
    'interpreted, so never treat it as binding or rely on it in either direction.',
    ...
    'no request; judge the candidate against the ruling instead.',
    '',
