...
export interface AuditVerdict {
   ...
   * action intent (eshyra-4ia4). A `give_item`/`claim_item`/`adopt_item`/`use_item`/`transfer_item`/`remove_item` call made merely to
...
const AUDIT_POLICY = [
  ...
  '  `give_item`, `claim_item`, `reacquire_item`, `adopt_item`, `use_item`, `transfer_item`, `remove_item`, `update_clock`, `set_plot_flag`, `set_world_fact`).',
  ...
  '  Identity-preserving repurchase is the one atomic exception: require a',
  '  successful `reacquire_item` with basis `repurchased` and an embedded nonempty',
  '  payment. A missing/failed payment or a separate unlinked `spend_currency` is',
  '  not evidence of repurchase custody or payment.',
  ...
  '  `claim_item`, `reacquire_item`, `adopt_item`, `use_item`, `transfer_item`, `remove_item`) to answer such a query is a violation: list that tool in',
...
export function buildAuditSystemPrompt(): string {
  return [
    ...
    'EXCEPTION: a successful `accept_ambiguity_precedent` may resolve an',
    'UNRESOLVED ambiguity by accepted action. Independently check the actual',
    'player input is an action (not a question), it clearly selects exactly one',
    'enumerated interpretation, the DM accepts it, the reason rules out the other',
    'interpretations, and the visible precedent matches. Reject that tool for',
    'canonical violations, contextual judgments, multiple/no matches, or an',
    'already active ruling. A valid proposal needs no request_ambiguity_ruling;',
    'it is prospective and commits only with this audited candidate.',
    'Otherwise, a candidate that asserts a definite outcome for an ambiguity listed as',
