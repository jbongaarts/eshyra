...
export function buildSystemPrompt(
  ...
  return [
    ...
    '- `remove_item` always requires the physical disposition. Use `destroyed` only when the item is actually destroyed; only a co-located dropped row remains under general pickup/destruction authority. `dropped`, `sold`, and `lost` clear held storage and preserve the physical row, state, and exact disposition, but sold/lost rows are not advertised or claimable as nearby loot. Use `claim_item` only when a character explicitly picks up a co-located dropped row. Use `reacquire_item` only for an explicitly adjudicated, co-located return of one exact sold/lost row, with a disposition-compatible basis and durable custody evidence. Use `transfer_item` for custody changes between holders, never `give_item`.',
    '- Use `adopt_item` only to explicitly recognize an already-held legacy row as one exact canonical magic item. Look up and supply the exact `packRef` and required `variantId`; never infer identity from the inventory display name. A review-required result quarantines identity, custody, attunement, and live-state writes pending explicit GM reconciliation; call `adopt_item` with `resolveReview: true` only after that reconciliation.',
