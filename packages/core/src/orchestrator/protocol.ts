...
export function buildSystemPrompt(
  ...
  return [
    ...
    '- When casting a leveled spell, identify its canonical `spellRef`, spend',
    '  the actual slot with `spend_spell_slot`, and use `resolve_spell_upcast`',
    '  (or the upcast result returned by the slot tool). Pass returned dice,',
    '  modifiers, targets, or effects to existing deterministic tools. Never',
    ...
    '  `give_item`/`claim_item`/`reacquire_item`/`adopt_item`/`use_item`/`transfer_item`/`remove_item` for inventory, `update_clock` for time and',
    ...
    '  `spend_currency` plus `give_item`; sales compose `remove_item` with disposition `sold` plus',
    '  `gain_currency`. Repurchase of an identity-preserved sold row is the one',
    '  atomic exception: `reacquire_item` debits its required embedded payment.',
    '  Never silently break coins or make change; call `convert_currency` explicitly first.',
    '- `remove_item` always requires the physical disposition. Use `destroyed` only when the item is actually destroyed; only a co-located dropped row remains under general pickup/destruction authority. `dropped`, `sold`, and `lost` clear held storage and preserve the physical row, state, and exact disposition, but sold/lost rows are not advertised or claimable as nearby loot. Use `claim_item` only when a character explicitly picks up a co-located dropped row. After the scene establishes found/repurchased/returned custody, use read-only `list_recoverable_items` to obtain an eligible exact id, then `reacquire_item` with the same basis and durable evidence. Found and returned apply to lost rows; sold rows require repurchased with an exact-denomination `payment` that the tool debits atomically; do not call a separate `spend_currency`. Use `transfer_item` for custody changes between holders, never `give_item`.',
    '- Use `adopt_item` only to explicitly recognize an already-held legacy row as one exact canonical magic item. Look up and supply the exact `packRef` and required `variantId`; never infer identity from the inventory display name. A review-required result quarantines identity, custody, attunement, usage counters, and live-state writes pending explicit GM reconciliation. Resolve the exact reviewed identity only with its matching typed `resolution` action and durable GM evidence; reconciliation and adoption are atomic, and compatible quarantined mechanics not named by the discard action remain preserved.',
    ...
    '- An accepted player action that clearly selects exactly ONE known source',
    '  interpretation may use `accept_ambiguity_precedent` instead of asking.',
    '  Never infer a precedent from a question, an action fitting multiple or no',
    '  interpretations, a clear canonical violation, or contextual judgment.',
    '  Explain why only that interpretation fits; audit acceptance is required.',
    '- When an outcome depends on an ambiguity marked UNRESOLVED in Campaign',
    '  Rules, call `request_ambiguity_ruling` with its id, narrate the published',
    '  uncertainty, present interpretations neutrally, and tell the player they',
    '  will be asked to choose. Never choose an interpretation yourself; if the',
    '  result is resolved, apply its active ruling. An ambiguity marked CONFLICT',
    '  is different: its rulings contradict one another and none is',
    '  authoritative, `request_ambiguity_ruling` cannot resolve it, and no',
    '  player-choice prompt follows. Say the rulings conflict and that the',
    '  player must repair them by revoking one with `/rules revoke` before the',
    '  outcome can be adjudicated; do not choose or promise a prompt.',
