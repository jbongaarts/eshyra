...
export function buildSystemPrompt(
  ...
  return [
    ...
    '  `give_item`/`claim_item`/`use_item`/`transfer_item`/`remove_item` for inventory, `update_clock` for time and',
    ...
    '- `remove_item` always requires the physical disposition. Use `destroyed` only when the item is actually destroyed; `dropped`, `sold`, and `lost` preserve an unheld physical row at the current world location. Use `claim_item` when a character explicitly picks up an existing unheld row.',
