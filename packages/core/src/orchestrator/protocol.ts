...
export function buildSystemPrompt(
  ...
  return [
    ...
    '- `remove_item` always requires the physical disposition. Use `destroyed` only when the item is actually destroyed; an unheld row can be destroyed only at the current campaign location. `dropped`, `sold`, and `lost` clear held storage and preserve an unheld physical row at the current world location. Use `claim_item` only when a character explicitly picks up a co-located unheld row; use `transfer_item` for custody changes between holders, never `give_item`.',
