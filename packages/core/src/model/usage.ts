...
/**
 * One audit verdict for a candidate turn response. These records make retry
 * churn measurable independently of model-call token records: one primary DM
 * candidate may require several model rounds, then one auditor pass, then a
 * retry when the verdict rejects.
 */
...
/** Receiver for per-candidate audit diagnostics. */
export interface TurnAuditSink {
...
export interface TurnOutcomeRecord {
  ...
  /** Auditor passes executed for this turn. */
  readonly auditorCallCount: number;
  /** Candidate responses generated by the primary DM. */
  readonly primaryDmCandidateCount: number;
  /** Primary DM model calls, including tool rounds, across all attempts. */
  ...
  /** Tool names repeated in an accepted retry after a rejected candidate. */
 ...
 */
export interface TurnDiagnosticsSink
  extends ToolUsageSink,
    TurnAuditSink,
    TurnOutcomeSink {}
...
export class NoopTurnDiagnosticsSink implements TurnDiagnosticsSink {
  ...
  recordAudit(_entry: TurnAuditRecord): void {}
