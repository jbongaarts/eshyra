...
export function enrichClassChapterRecords(input: {
  ...
  // and the FEATURE_TABLE_REFS pass below. Spellcasting/Pact Magic prose
  // ownership no longer needs a canonicalization pass here: the feature
  // parser now emits ONE record per class-table feature, with the printed
  // subheadings carried as `data.sections` on that same record
  // (eshyra-o9bd.19.2.2.4 — replaces the eshyra-o9bd.4 copy-back this pass
  // used to perform).
