...
| `creature`  | Implemented (317 records: 296 monsters + 21 NPCs). Stat-block parser extracts every creature from the Monsters section AND Appendix MM-A into `kind=creature` records satisfying the `dnd5e-srd` creature kindSchema: `size`, `type` (subtype parenthetical preserved; validation applied to the bare type word), `alignment`, structured statline objects (eshyra-o9bd.18.6) — `armorClass` `{ value, source?, condition?, variants?, sourceText }` preserving armor-source parentheticals ("natural armor", "leather armor, shield"), spell-adjusted/conditional alternates (Mage's "15 with mage armor", Ankheg's "11 while prone"), and the dual-form lycanthrope ACs (whose "in <beast> and/or hybrid form" clause wraps onto the next extracted line and is re-joined), and `hitPoints` `{ value, formula }` carrying the printed average plus the printed dice formula verbatim (matching the inline `stat-block` kind) — `speed` (mode→feet map of UNCONDITIONAL modes only; the unlabeled base speed keyed as `walk`) with the printed `(hover)` preserved as an optional literal-true `hover`, the four lycanthrope form parentheticals preserved as `speedVariants` `[{ condition, speed }]` (so the Werebear no longer leaks a bogus unconditional `climb: 30`), and the verbatim Speed value kept as `speedSourceText`, `challengeRating` (bare fraction/integer), and `abilityScores`. The AC/HP/Speed value grammars are anchored and fail closed: any statline content outside the reviewed shapes throws instead of being flattened, and the pack-level `creature-statline-fidelity` audit gate (eshyra-o9bd.18.6.4) re-verifies the structured fields against the verbatim `sourceText`/`speedSourceText` (AC token-residue check, HP formula floored-mean math, exact Speed-line re-render). Records also carry the optional keyed defensive/sense fields the SRD prints between the ability-score row and the Challenge line — `savingThrows`, `skills`, `damageVulnerabilities`, `damageResistances`, `damageImmunities`, `conditionImmunities`, `senses`, `languages` — each preserved verbatim and emitted only when its source label is present (eshyra-ez6v / eshyra-4a7.5). The 33 heading-only family/group labels in the main Monsters chapter are preserved as optional source-derived `familyPath` arrays on their 116 member records (eshyra-4a7.10.2): ordinary groups use one segment (`["Angels"]`), while dragon records preserve category and color nesting (`["Dragons", "Chromatic Dragons", "Black Dragons"]`). Reviewed last-member boundaries prevent a family from leaking onto the following standalone monster; Appendix MM-A creatures and MM-B NPCs carry no `familyPath`. Those values are reassembled by label-position slicing over the bounded ability-row→Challenge region, so a value that wraps across extracted lines (Deva's Damage Resistances) or a next-field label the PDF column flow merged onto the previous line (Wereboar's `… silvered weapons Senses …`) is captured correctly; the singular `Damage Resistance` a conditional block prints (Archmage's stoneskin) is accepted as an alias. After the keyed fields, the narrative body is parsed into named `{name, text}` entries (eshyra-yevt / eshyra-4a7.5): an implicit `traits` run, the header-delimited `actions` and `reactions` sections, and `legendaryActions` (`{ description?, entries }` — the intro paragraph plus options). Each is optional and emitted only when the creature prints that section; entry names keep their usage parenthetical ("Enslave (3/Day)", "Fire Breath (Recharge 5-6)") and bodies preserve attack bonuses, damage expressions, save DCs, and spell lists verbatim (a Spellcasting/Innate Spellcasting trait carries its full spell list as text). Every narrative entry additionally carries a typed `mechanics` projection where a reviewed grammar matches (eshyra-o9bd.18.7.3): attack lead-ins (incl. "one creature" targets and flat no-dice damage like the Bat's "1 piercing damage"), save DCs, dice damage, condition relations, dice recharge plus name-parenthetical use economies (`usage.perDay` / `usage.rechargeAfterRest` / `usage.legendaryActionCost`), and effect grammars — legendary resistance, regeneration (with typed damage-type suppression), multiattack counts, healing dice, advantage/disadvantage modifiers on saves/checks/attack rolls (Magic Resistance, Keen Senses, Blood Frenzy, Sunlight Sensitivity, Gnome Cunning), breathing/jump traits, Parry AC bonuses, magical weapon attacks, form-alteration/turn-undead immunities, Siege Monster double damage, legendary named-attack and Detect references, and verbatim triggered-effect markers. A `Spellcasting` / `Innate Spellcasting` trait projects a structured `mechanics.spellcasting` object (mode, ability, save DC/attack bonus, caster level/list class, component waivers, and at-will / N-per-day / slot-level groups whose spells are fail-closed resolved `spell:` refs — an unresolvable token drops the whole projection, never a partial list). Grammars fail closed: prose outside a reviewed shape contributes no projection, and the audit-bundle readiness report tracks nested entry coverage with explicit accepted-prose dispositions. Derived `mechanics` subtrees are excluded from the source-region ledger's emission index (they are computed from sibling source text and would both break needle contiguity and mask dropped lines). Splitting mirrors the `parseAncestries` "Name. body" splitter (a lead-in printed alone on its line, body wrapped to the next — Wererat Hand Crossbow, Chain Devil Animate Chains — also opens an entry; eshyra-o9bd.19.2.2.1, enforced corpus-wide by the source-derived attack lead-in gate in `creatureAttackLeadIns.ts`) with a sentence-completeness guard (so a wrapped sentence beginning with a capitalized phrase like "Constitution saving throw. …" is not mis-promoted) plus a spell-list guard (so the trait after a spell list still opens). Structural heading lines — SRD creature-group headings ("Angels", "Dragons"), running page headers ("Monsters (B)"), and leaked creature names — are dropped from the body by font height (>= ~12pt, vs. <= 10.8pt for stat-block content). The two boxed `Variant: …` sidebars the SRD prints in the creature chapters (Diseased Giant Rats p378, Insect Swarms p391) end the preceding creature's narrative (so their bold-lead-in lines do not pollute it — e.g. the Giant Rat's duplicate Bite) and are attached as a `variants` `{name, text}` array on the creature each one modifies via a reviewed caption→target map (Giant Rat, Swarm of Insects); a new unmapped `Variant:` caption fails closed (eshyra-70xr). SRD 5.1 prints no per-creature Lair Actions / Regional Effects (those terms appear only in the general Legendary Creatures rules on p260); the Half-Dragon Template and dragon reference tables are owned by `eshyra-4a7.10`. A stat block is confirmed by its size/type/alignment meta line plus the AC/HP/ability-table signature. **Appendix MM-B: Nonplayer Characters** (the 21 generic NPC stat blocks — Acolyte, Bandit Captain, Berserker, …) is imported too (loreweaver-bn0): an NPC stat block has the identical shape and is equally encounter-usable, so it emits under the same `creature` kind via the same parser, distinguished by a `data.category: 'npc'` discriminator. Monster records carry no `category` field — its absence means "monster" — so the monster/NPC provenance baselines remain independent. Section anchors: `monsters` (`startHeading: /^Monsters$/`, `requireEndHeading: true`), `miscellaneousCreatures` (Appendix MM-A, best-effort), and `nonplayerCharacters` (Appendix MM-B, best-effort start, runs to EOF). `runImporter` fails closed via two independent coverage guards: an empty Monsters parse always throws, the real import (CLI + `verify:dnd5e-srd-pack`) validates monster names against the exact `EXPECTED_SRD_5_1_CREATURE_NAMES` set (296) and NPC names against `EXPECTED_SRD_5_1_NPC_NAMES` set (21) — any missing, renamed, or spuriously-extracted entry throws by name (`CreatureCoverageError` / `NpcCoverageError`) and writes nothing (loreweaver-0m9.5.14, loreweaver-bn0). Both sets are reviewed, checked-in regression baselines (the monster candidate is generated via `npm run generate:dnd5e-srd-creature-names`, reviewed against the SRD source, then committed), not runtime-derived expected data. Fixture pipelines that exercise a reduced Monsters section omit the exact sets and rely on the empty-result guard or the coarse `minCreatureCount` floor; a fixture without an MM-B appendix degrades to zero NPCs. |
...
| `feature`   | Implemented. Parser extracts class- and subclass-granted features from the Classes-chapter slice into `kind=feature` records satisfying the `dnd5e-srd` feature kindSchema (`data.source` keyed to the grantor `class:<slug>` / `subclass:<slug>`, integer `data.level`, `data.description`). Grantor context is tracked by known base-class and subclass names (the same anchors `parseSubclasses` uses). Feature identity and grant level come primarily from class/subclass progression-table rows; a leading prose clause like "At 3rd level" is used as a fallback ONLY for a subclass grantor (eshyra-o9bd.19.2.2.4 D1) — a class grantor with no table anchor is never promoted to a feature, since that was the root cause of the retired `feature:{cleric,druid,sorcerer,wizard}:cantrips` / `feature:wizard:spellbook` parser-artifact records (a class feature's own printed 12pt subheading, e.g. Wizard's "Cantrips" under "Spellcasting", opening with an "At 1st level, …" clause). Unanchored title-case option subheadings inside feature bodies remain in the parent feature description, but a gray callout-box heading (a generic class/DM sidebar at its own font-height tier, e.g. Wizard "Your Spellbook" after Overchannel) bounds the last feature's body. The dedicated class-callout parser owns that bounded prose and emits it as a standalone parent-qualified `rule` record rather than leaving it inside the feature (loreweaver-6fw, loreweaver-0m9.5.23). `runImporter` fails closed via a feature-coverage guard: an empty feature parse (or a count below `minFeatureCount`, which the CLI sets to `MIN_EXPECTED_SRD_5_1_FEATURES`) throws `FeatureCoverageError` and writes nothing. The two feature-owned class tables carry `data.tableRefs` (eshyra-4a7.6): `feature:cleric:destroy-undead` → `table:destroy-undead` and `feature:druid:wild-shape` → `table:beast-shapes`; the flattened table rows are trimmed out of those two descriptions while the prose that introduces the table is kept. A class-grantor feature named exactly "Spellcasting" or "Pact Magic" additionally carries `data.sections` — one `{ name, text }` entry per printed 12pt subheading under it ("Cantrips", "Spell Slots", "Spellcasting Ability", …), verbatim, in print order; `data.description` holds only the intro prose before the first subheading (eshyra-o9bd.19.2.2.4 D2). The Spellcasting/Pact Magic section parity gate (`spellcastingSections.ts`, `assertSpellcastingSections`) independently re-derives both the printed subheading list (by font-height tier, not by re-running `parseFeatures`) and the class-table membership of every `class:*`-sourced feature record, and fails closed on any drift. Shares the `classes` slice (ADR 0009 / loreweaver-0m9.5.18). |
...
| `equipment` | Implemented (219 records). Parser projects the Equipment chapter and the Mounts and Vehicles section into per-item `kind=equipment` records, all carrying `category` plus verbatim `cost`/`weight` where the source lists them: armor (13: `ac`, `armorType` from the AC cell, `stealthDisadvantage`, optional `strengthRequirement`), weapons (37: `damageDie`, `damageType`, `properties[]`), tools (36: the Tools table's 35 priced rows plus its closing "Vehicles (land or water)" row, whose "*" cost/weight cells are not invented — the record carries no `cost`/`weight` and its `description` is the table footnote "See the “Mounts and Vehicles” section."; any other unrecognized Tools-table line throws `ToolsTableRowError`; `runImporter` then re-reads the printed Tools rows from the source independently of the parser and requires exact name/cost/weight parity with the emitted tool records — `assertToolsTableRowParity` in `toolsTableRows.ts`, eshyra-o9bd.19.2.2.2), Adventuring Gear (99) + Tack/Harness/Drawn Vehicles (13) as `category='gear'`, Equipment Packs (7: `category='pack'`, verbatim contents `description`), mounts (8: `category='mount'`, `speed`, `carryingCapacity`), and waterborne vehicles (6: `category='vehicle'`, `speed`). **110 records carry source descriptions**: the seven packs, the 101 armor, special-weapon, adventuring-gear, and tool records described in the chapter, and two table footnotes (the Backpack strap note and the Tools table's Vehicles row). Shared category prose (arcane/druidic focuses, holy symbols, artisan's tools, gaming sets, and musical instruments) is attached to each owning row; multi-page provenance cites both the description and table-row pages. The real SRD 5.1 PDF extracts Armor and Weapons as two physical columns (left/right blocks zipped positionally) while Tools extracts row-major. **Adventuring Gear (loreweaver-4zu)** is the hard case: its left column's item names arrive as one bare run, then the left cost/weight values interleave line-by-line with the right column's complete rows, and four category-header rows (Ammunition, Arcane focus, Druidic focus, Holy symbol) carry no value. Reconstruction removes those four reviewed headers from the name run, then length-checks and zips the de-headered names against the left values (a mismatch throws `EquipmentColumnMismatchError('Gear', …)`); the right column's rows are self-contained. The **Container Capacity** table is attached as a verbatim `capacity` field to the matching gear record via a reviewed name-alias map (an unmatched row throws `ContainerCapacityError`). The Tack table's "Saddle" sub-header variants are qualified to "Saddle, <variant>"; its non-priced "Barding ×4 ×2" multiplier row is skipped. Section anchors: `equipment` (`startHeading: /^Equipment$/`, `requireEndHeading: true`) and `mountsAndVehicles` (`startHeading: /^Mounts and Vehicles$/`; end-bounded by "Trade Goods" but `requireEndHeading` is off because `parseMountsAndVehicles` is internally header-bounded). |
...
- `parseTraps.ts` -- narrowed "Traps"-section text -> `TrapExtraction[]` by each
  ...
  tables. Body is re-flowed prose; `category: "trap"` identifies the hazard
  sub-family and `trapType` records the subtitle. Emitted under the `hazard`
  kind (loreweaver-hvp). The general trap-rules prose is
...
- `parseDiseases.ts` -- narrowed "Diseases"-section text -> `DiseaseExtraction[]`
  ...
  Plague, Sight Rot). The leading guidance prose and "Sample Diseases" caption
  are emitted separately as rules. Body is re-flowed prose. Emitted under the `hazard` kind with
...
- `parseEquipmentGuidance.ts` -- source-bounded Equipment, Mounts/Vehicles,
  Trade Goods, Expenses/Lifestyle, Food/Drink/Lodging, and Services prose into
  `RuleExtraction[]`, excluding table rows and item descriptions owned by
  equipment records.
...
column-header fragments are `ignore`d as table internals; the 86 -> 93
(eshyra-o9bd.19.2.2.4: "Cantrips" (6 occurrences) and "Spellbook" (1) join
SPELLCASTING_BOILERPLATE, riding in their owning feature's body like every
other subheading here instead of falling to the bare-name auto-match)
feature-option and spellcasting-boilerplate leaf subheadings (Fighting
Styles, Metamagic, ki options, Pact Boons, Eldritch Invocations,
"Cantrips"/"Spellbook"/"Ritual Casting"/"Spellcasting
...
| `sentientMagicItems` | `/^Sentient Magic Items$/`                     | `/^Artifacts$/`                                         | `true` (best-effort start) | `true`     |
...
- `source-coverage.json` — `{ summary, entries }` where `summary` rolls up
  ...
  `taxonomy:creature.familyPath:<path>` | `known-gap:<beadId>` |
  `unaccounted`.
