...
| `rule`      | Implemented (**299 records**): the existing core, spellcasting, gamemastering, class-callout, Beyond 1st Level, Magic Items, Traps, Monsters, Backgrounds, and Spellcasting Services rules, plus the standalone `Variant: Skills with Different Abilities` and `Variant: Encumbrance` gameplay rules; 16 source-bounded Races/Equipment guidance records (eshyra-4a7.10.1): the Racial Traits intro and seven category explanations, Getting Into and Out of Armor, Weapons Guidance with Weapon Proficiency / Weapon Properties / Improvised Weapons / Silvered Weapons / Special Weapons, and the Self-Sufficiency sidebar; the Half-Dragon Template guidance record (eshyra-4a7.10.3); the 8 Magic Items "Sentient Magic Items" DM construction-guidance records (eshyra-4a7.10.4): the Sentient Magic Items intro plus Creating Sentient Magic Items, Abilities, Communication, Senses, Alignment, Special Purpose, and Conflict; the Appendix MM-B "Customizing NPCs" guidance record (eshyra-4a7.10.6), carved off the front of the NPC appendix slice by truncating before the first ("Acolyte") stat block so its prose is emitted without absorbing the adjacent stat blocks; and the Appendix MM-A "Miscellaneous Creatures" intro record (`rule:appendix-mm-a-miscellaneous-creatures`, eshyra-76b7), carved off the front of the MM-A appendix slice by truncating before the first ("Ape") stat block — header-less body prose that the heading-hierarchy parser cannot emit, captured by the dedicated `parseMiscellaneousCreaturesIntro` bounded-prose parser so it is no longer dropped or bled into the preceding creature's last action. The four racial headings that collide with later rule names use `rule:racial-traits-*` keys; the sentient-item Senses and Alignment headings likewise collide with the Monsters/Beyond-1st-Level rules and use `rule:creating-sentient-magic-items-*` keys, so explicit source-coverage record rules keep each colliding source heading on its own record. Equipment and sentient-item table-cell lines are removed only from these prose-only slices; the Donning and Doffing Armor, Weapons, and the four sentient-item roll tables remain represented by table/equipment records. The real import fails closed against the exact combined `EXPECTED_SRD_5_1_RULE_KEYS` set. Full body text is stored in `data.text`. |
