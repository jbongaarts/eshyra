...
- A capability declaration qualifies its identity/revision and its declared
  inputs, exclusions, and residual interpretation.  It does **not** qualify
  availability for a candidate, operation, or input.  That bounded
  `available`/`blocked` claim requires the capability owner's
  subject-specific preflight or admission evidence; declaration-only is
  `not-evaluated-offline`/indeterminate for availability.  Discovery quotes
  each boundary and does not redefine it.
