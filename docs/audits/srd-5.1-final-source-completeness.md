...
**Bead:** `eshyra-4a7.9`\
**Refresh bead:** `eshyra-69en`\
**Source commit:** `0a1c441da5523c5042977229ff421000844ecea5`\
...
drift. The follow-up importer fixes from PR #218 are present and verified; no
additional importer or generated-record changes are required.
...
npm run audit-bundle:dnd5e-srd -- \
  .audit-bundles/eshyra-69en-final \
  /tmp/eshyra-69en-final.zip
```

The archive is 6.4 MiB with SHA-256:

```text
7ecc2c4eded7683390cd7f6c964f9b3baeb1fbbb755fbf63f54304cb692847f7
```
...
The three findings identified after the original audit are fixed in the
refreshed pack and source-coverage artifacts:

- `spell:hunters-mark` preserves the source name `Hunter’s Mark` and now has
  `classes: ["Ranger"]`, resolving the ASCII/curly-apostrophe spell-list match.
- The page 395 Appendix MM-B `Acolyte` stat block maps to
  `record:creature:acolyte`, not `record:background:acolyte`.
- The page 398 Appendix MM-B `Druid` stat block maps to
  `record:creature:druid`, not `record:class:druid`.

The importer tests also require every source spell-list entry to resolve to an
emitted spell and require source inventory stat blocks to resolve to creature
or stat-block records unless a named source-backed exception is present.
...
All source inventory entries identified as stat blocks resolve to creature or
standalone stat-block records.
...
The fresh bundle completed successfully and captured successful runs of:
...
After updating this report, the standalone `npm run verify:dnd5e-srd-pack` and
`npm run verify:worktree` commands were repeated successfully. The
reproducibility check confirms that a fresh importer run matches the committed
manifest, records, source inventory, and source coverage artifacts
byte-for-byte.
